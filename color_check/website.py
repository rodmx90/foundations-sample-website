...
def show_color():
    ...
    # - check if the color exists in our list, return the hex code if it does -> get_color_code.py
    ...
    user_submitted_string = request.form.get("color_name")
    color_hex_code = get_color_code(user_submitted_string)

    return render_template('color.html', page_title="Show Color", color_hex_code=color_hex_code, user_submitted_string=user_submitted_string)
